...
void sha_final(SHA_INFO * __capability sha_info)
{
    ...
    if (count > 56) {
	memset((BYTE * __capability)sha_info->data + count, 0, 64 - count);
#ifdef LITTLE_ENDIAN
	byte_reverse(sha_info->data, SHA_BLOCKSIZE);
#endif /* LITTLE_ENDIAN */
	sha_transform(sha_info);
	memset((void * __capability)sha_info->data, 0, 56);
    } else {
	memset((BYTE * __capability)sha_info->data + count, 0, 56 - count);
    }
#ifdef LITTLE_ENDIAN
    byte_reverse(sha_info->data, SHA_BLOCKSIZE);
#endif /* LITTLE_ENDIAN */
...
void sha_stream(SHA_INFO * __capability sha_info, char * __capability fin, size_t fin_size)
{
    ...
    BYTE * __capability datacap = data;
